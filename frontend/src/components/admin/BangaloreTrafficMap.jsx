...
export default function BangaloreTrafficMap() {
  ...
  const [selectedCity, setSelectedCity] = useState('gujarat');
  ...
  const [routeOrigin, setRouteOrigin] = useState('S.G. Highway - Thaltej, Ahmedabad');
  const [routeDestination, setRouteDestination] = useState('Infocity Circle, Gandhinagar');
  const [selectedRouteType, setSelectedRouteType] = useState('ai_optimal'); // 'ai_optimal' | 'congested_primary' | 'alternate'
  const [isRoutingActive, setIsRoutingActive] = useState(false);

  // Pre-configured Gujarat corridors for routing
  const routePresets = [
    { label: 'Ahmedabad ➡️ Gandhinagar', origin: 'S.G. Highway - Thaltej, Ahmedabad', dest: 'Infocity Circle, Gandhinagar' },
    { label: 'Vadodara ➡️ Surat', origin: 'Alkapuri, Vadodara', dest: 'Athwa Gate, Surat' },
    { label: 'Surat ➡️ Rajkot', origin: 'Varachha Road, Surat', dest: 'Kalawad Road, Rajkot' },
    { label: 'Bhavnagar ➡️ Jamnagar', origin: 'Ghogha Circle, Bhavnagar', dest: 'Bedi Road, Jamnagar' }
  ];
  ...
  const fetchBangaloreData = async () => {
    try {
      ...
      if (zonesRes.data?.zones) {
        ...
        if (!selectedZone) {
          const defaultSilk = zonesRes.data.zones.find(z => z.zone_id === 'GJ-AHM-01') || zonesRes.data.zones[0];
  ...
  const handleOpenGoogleDirections = () => {
    const origin = selectedZone?.name || routeOrigin || 'S.G. Highway - Thaltej, Ahmedabad';
    let destination = routeDestination;

    // Smart corridor destination inference based on selected origin
    if (origin.toLowerCase().includes('ahmedabad')) {
      destination = 'Infocity Circle, Gandhinagar';
    } else if (origin.toLowerCase().includes('vadodara')) {
      destination = 'Athwa Gate, Surat';
    } else if (origin.toLowerCase().includes('surat')) {
      destination = 'Kalawad Road, Rajkot';
  ...
  const handleOpenExternalGoogleMaps = () => {
    const origin = routeOrigin || selectedZone?.name || 'S.G. Highway - Thaltej, Ahmedabad';
    const destination = routeDestination || 'Infocity Circle, Gandhinagar';
    const url = `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(origin + ' Gujarat')}&destination=${encodeURIComponent(destination + ' Gujarat')}`;
  ...
  const getGoogleMapsEmbedUrl = () => {
    ...
    if (mapViewMode === 'routing') {
      const origin = routeOrigin ? `${routeOrigin}, Gujarat` : "S.G. Highway - Thaltej, Ahmedabad, Gujarat";
      const destination = routeDestination ? `${routeDestination}, Gujarat` : "Infocity Circle, Gandhinagar, Gujarat";
      return apiKey
    ...
    let target = selectedZone ? `${selectedZone.name}, Gujarat` : "S.G. Highway - Thaltej, Ahmedabad, Gujarat";
    if (searchQuery.trim()) {
      target = `${searchQuery.trim()}, Gujarat`;
    }
    if (apiKey) {
  ...
  const keyCorridors = BANGALORE_LANDMARKS.slice(0, 7).map((landmark) => ({
    id: landmark.id,
    name: landmark.name.split(',')[0],
    fullName: landmark.name,
    icon: '🚦',
    level: landmark.defaultLevel
  }));
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <div className="bg-slate-50 p-1.5 rounded-2xl border border-slate-200 flex items-center">
              <button
                onClick={() => setSelectedCity('gujarat')}
              ...
              >
                <Building2 className="w-3.5 h-3.5" /> Gujarat (Primary)
              </button>
              <button
              ...
              <button
                ...
                onClick={() => {
                  const targetZone = zones.find(z => z.name === corridor.fullName || z.zone_id === corridor.id) || {
                    ...
                    latitude: BANGALORE_LANDMARKS.find(l => l.id === corridor.id)?.lat || 23.0507,
                    longitude: BANGALORE_LANDMARKS.find(l => l.id === corridor.id)?.lng || 72.5110,
                    average_speed: 8.5,
          ...
          <div className="bg-[#0F172A] rounded-3xl overflow-hidden shadow-2xl border border-slate-800 relative h-[680px] flex flex-col justify-between select-none">
            ...
            <div className="absolute top-4 left-4 right-4 z-20 flex items-center justify-between gap-3 pointer-events-none">
              ...
              <div className="bg-slate-900/95 backdrop-blur-md p-1.5 rounded-2xl border border-slate-700 pointer-events-auto flex items-center gap-1.5 shadow-xl">
                ...
                <button 
                  onClick={() => { setMapZoom(8); setSelectedZone(zones.find(z => z.zone_id === 'GJ-AHM-01') || zones[0]); }}
                  ...
                  <div className="relative mx-auto flex h-full max-w-3xl flex-col justify-between">
                    <div className="flex items-center justify-between">
                      <div className="rounded-xl border border-slate-200 bg-white/95 px-3 py-2 shadow-sm">
                        ...
                        <p className="text-xs font-bold text-slate-800">{selectedZone?.name || 'Gujarat network'}</p>
                      </div>
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                      <div className="rounded-2xl border border-white bg-white/95 p-4 shadow-sm">
                        ...
                        <p className="mt-1 text-sm font-black text-slate-900">{selectedZone?.name || 'Gujarat'}</p>
                      </div>
                      <div className="rounded-2xl border border-white bg-white/95 p-4 shadow-sm">
