...
export default function ParkingBooking({ user }) {
  ...
  const gujaratHubs = [
    { name: 'S.G. Highway - Thaltej Parking, Ahmedabad', price: 60 },
    { name: 'Infocity Circle Parking, Gandhinagar', price: 50 },
    { name: 'Alkapuri Parking, Vadodara', price: 70 },
    { name: 'Athwa Gate Parking, Surat', price: 60 },
    { name: 'Varachha Road Parking, Surat', price: 50 },
    { name: 'Kalawad Road Parking, Rajkot', price: 45 },
    { name: 'Ghogha Circle Parking, Bhavnagar', price: 40 },
    { name: 'Bedi Road Parking, Jamnagar', price: 45 }
  ];

  const allowedGujaratHubNames = new Set(gujaratHubs.map(h => h.name));

  const zonesMap = spots.reduce((acc, spot) => {
    const zoneKey = spot.zone || spot.zoneId;
    if (zoneKey && allowedGujaratHubNames.has(zoneKey)) {
  ...
  gujaratHubs.forEach(hub => {
    if (!zonesMap[hub.name]) {
      zonesMap[hub.name] = { id: hub.name, name: hub.name, pricePerHour: hub.price, spots: [] };
    }
  });

    // Set initial zone to the primary Gujarat hub if none selected
  if (!selectedZone) {
      setSelectedZone('S.G. Highway - Thaltej Parking, Ahmedabad');
  }
